...
admissions_death <- 
  mutate(admissions_origin, hospitaltime =  admissions_origin$dischtime - admissions_origin$admittime, edtime = admissions_origin$edouttime - admissions_origin$edregtime) %>% 
...
par(mfrow = c(2,2))
living_lm1 <- 
  glm(hospital_expire_flag ~ admission_type, data = admissions_death) %>% 
  plot()
summary(living_lm1)
...
living_lm2 <- 
  glm(hospital_expire_flag ~ admission_location, data = admissions_death) %>% 
  plot()
summary(living_lm1)
...
living_lm3 <- 
  glm(hospital_expire_flag ~ insurance, data = admissions_death) %>% 
  plot()
summary(living_lm3)
...
living_lm4 <- 
  glm(hospital_expire_flag ~ language, data = na.omit(select(admissions_death, living, language))) %>% 
  plot()
summary(living_lm4)
...
living_lm5 <- 
  glm(hospital_expire_flag ~ religion, data = admissions_death) %>% 
  plot()
summary(living_lm5)
...
living_lm6 <- 
  glm(hospital_expire_flag ~ marital_status, data = admissions_death) %>% 
  plot()
summary(living_lm6)
...
living_lm7 <- 
  glm(hospital_expire_flag ~ ethnicity, data = admissions_death) %>% 
  plot()
summary(living_lm7)
...
living_lm10 <- 
  glm(hospital_expire_flag ~ has_chartevents_data, data = admissions_death) %>% 
  plot()
summary(living_lm10)
...
living_lm11 <- 
  glm(hospital_expire_flag ~ hospitaltime, data = admissions_death) %>% 
  plot()
summary(living_lm11)
...
living_lm12 <- 
  glm(hospital_expire_flag ~ edtime, data = na.omit(select(admissions_death, living, edtime))) %>% 
  plot()
summary(living_lm12)
...
\item $\hat{Living}$ = \item $\hat{\beta_0}$ + \item $\hat{\beta_1}$ Admission.Location + \item $\hat{\beta_2}$ Insurance + \item $\hat{\beta_3}$ Religion + \item $\hat{\beta_4}$ Marital.Status + \item $\hat{\beta_5}$ Ethnicity + \item $\hat{\beta_6}$ Hospital.Expire.Flag + \item $\hat{\beta_7}$ Has.ChartEvents.Data + \item $\hat{\beta_8}$ Hospital.Time + \item $\hat{\beta_9}$ Edtime 
