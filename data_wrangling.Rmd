...
mutate(admissions_death, fitted = fitted(living_mlr)) %>%
  ggplot(., aes(y = , x = )) + 
  geom_point() + 
...
library(DAAG)
cv.lm(df = admissions_death, living_mlr, m = 5) # 5 fold cross-validation



```{r}
step <- step(living_mlr)
broom::tidy(step)
step$anova
...
library(relaimpo)
calc.relimp(admissions_death, type=c("lmg","last","first","pratt"),
   rela=TRUE)
```
